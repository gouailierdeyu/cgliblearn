...
/**
 * 当调用被拦截的方法时，由Enhancer生成的类将此对象传递给注册的MethodInterceptor对象。
 * 它既可用于调用原始方法，也可用于在相同类型的不同对象上调用相同方法。
...
public class MethodProxy {
    ...
    /**
     * 返回由CGLIB创建的合成方法的名称，
     * 该合成方法被invokesSuper用于调用父类(未截获的)方法实现。
     ...
     * @param obj 不同的对象（可以把原始的被代理的子类传进来）
     *            如果使用MethodInterceptor的第一个参数传递的对象，则会导致递归。
     *           the compatible object; recursion will result if you use the object passed as the first
     * argument to the MethodInterceptor (usually not what you want)
     * @param args 方法参数 the arguments passed to the intercepted method; you may substitute a different
     ...
     * @param obj 就是子类对象，必须是MethodInterceptor的第一个参数 the enhanced object, must be the object passed as the first
     * argument to the MethodInterceptor
     * @param args 方法参数the arguments passed to the intercepted method; you may substitute a different
     * argument array as long as the types are compatible
     * @see MethodInterceptor#intercept
     * @throws Throwable the bare exceptions thrown by the called method are passed through
     * without wrapping in an <code>InvocationTargetException</code>
